...
import { pedidosAPI, authAPI } from '../api/axios'
...
export default function Perfil() {
  ...
  useEffect(() => {
    const loadPedidos = async () => {
      try {
        ...
        setPedidos(Array.isArray(data) ? data : data.pedidos || data.orders || [])
      } catch {
        // silent
      } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const msg = err.response?.data?.mensaje || 'Error en actualitzar el perfil'
  ...
  const imgSrc = user?.imatge
    ? user.imatge.startsWith('http') ? user.imatge : `${API_BASE}/${user.imatge}`
    : null

  const rolLabel = { admin: 'Administrador', editor: 'Editor', usuari: 'Usuari' }[user?.rol] || user?.rol || 'Usuari'
  const rolColor = { admin: 'badge-gold', editor: 'badge-burgundy' }[user?.rol] || 'badge bg-stone-800/40 text-stone-400 border border-stone-700/40'
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center gap-6 mb-10 pb-8 border-b border-stone-800/60">
          ...
          <div className="flex-1">
            <div className="flex flex-wrap items-center gap-3 mb-1">
              <h1 className="font-display text-3xl text-stone-100">{user?.nombre || 'Usuari'}</h1>
        ...
        <div className="flex border-b border-stone-800/60 mb-8 gap-6">
          {['perfil', 'comandes'].map((tab) => (
            ...
            >
              {tab === 'perfil' ? 'Dades Personals' : `Les Meves Comandes (${pedidos.length})`}
            </button>
          ))}
        </div>
        ...
        {activeTab === 'perfil' && (
          <div className="glass-panel p-8 max-w-lg">
            <div className="flex items-center justify-between mb-6">
              <h2 className="font-display text-xl text-stone-100">Informació del Compte</h2>
              {!editMode && (
                <button onClick={() => setEditMode(true)} className="btn-ghost text-xs py-1.5 px-4">
                  Editar
                </button>
              )}
            </div>
            ...
            {editMode ? (
              <div className="space-y-4">
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button
                    onClick={() => { setEditMode(false); setForm({ nombre: user?.nombre || '', email: user?.email || '' }) }}
          ...
          <div>
            {loadingPedidos ? (
              ...
              <div className="text-center py-16">
                ...
                <p className="font-display text-2xl text-stone-400 mb-2">Encara no has fet cap comanda</p>
                <p className="text-stone-500 font-body text-sm mb-6">Explora el catàleg i descobreix els nostres productes</p>
...
function InfoRow({ label, value }) {
  return (
    <div className="flex items-start justify-between py-3 border-b border-stone-800/40 last:border-0">
      <span className="text-xs uppercase tracking-widest text-stone-500 font-body">{label}</span>
      <span className="text-sm font-body text-stone-200 text-right max-w-[60%]">{value || '—'}</span>
    </div>
  )
}
...
function PedidoCard({ pedido }) {
  ...
  const date = new Date(pedido.createdAt || pedido.fecha).toLocaleDateString('ca-ES', {
    day: '2-digit', month: 'long', year: 'numeric'
  })

  const statusLabel = {
    pendent: 'Pendent', processat: 'Processat', enviat: 'Enviat',
    entregat: 'Entregat', cancel·lat: 'Cancel·lat',
  }[pedido.estat || pedido.estado] || pedido.estat || pedido.estado || 'Pendent'
    ...
    <div className="card">
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <span className={`badge text-[9px] uppercase tracking-widest border font-body bg-cellar-800/60 ${statusColor}`}>
            {statusLabel}
          </span>
        </div>
      ...
      {open && pedido.productos && (
        <div className="border-t border-stone-800/60 px-5 py-4">
          <p className="text-[10px] uppercase tracking-widest text-stone-500 font-body mb-3">Articles</p>
          <div className="space-y-2">
            {pedido.productos.map((item, i) => (
              <div key={i} className="flex items-center justify-between text-sm font-body">
                <span className="text-stone-300">
                  {item.producto_id?.nombre || item.nombre || item.producto_id || 'Producte'}
                  <span className="text-stone-600 ml-1">({item.producto_modelo || item.tipo})</span>
                </span>
                <span className="text-stone-400">× {item.cantidad || item.quantitat || 1}</span>
              </div>
            ))}
          </div>
        </div>
