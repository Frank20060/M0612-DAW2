...
import { vinosAPI, cervesesAPI, usuarisAPI, IMG_URL } from "../api/axios";
...
export default function Dashboard() {
  ...
  const handleDetallesChange = (field, value) => {
    setProductForm((p) => ({
      ...p,
      detalles: { ...p.detalles, [field]: value },
    }));
  };
                ...
                <table className="w-full text-sm font-body">
                  ...
                  <tbody className="divide-y divide-stone-800/40">
                    {usuaris.map((u) => (
                      <tr
                        key={u._id}
                        className="hover:bg-cellar-900/40 transition-colors"
                      >
                        ...
                        <td className="py-4 px-4">
                          <select
                            value={u.rol || "usuari"}
                            onChange={(e) =>
                              handleRolChange(u._id, e.target.value)
                            }
  ...
  return (
    <div className="glass-panel p-5 flex items-center gap-4">
      ...
      <div>
        ...
        <p className="text-[10px] uppercase tracking-widest text-stone-500 font-body">
          {label}
        </p>
      </div>
    </div>
...
function ProductSection({
  ...
  const imgSrc = (p) => {
    ...
    return p.imatge.startsWith("http")
      ? p.imatge
              ...
              <tr className="border-b border-stone-800/60">
                {["Producto", "Precio", "Stock", "Graduación", "Acciones"].map(
                  (h) => (
                    <th
                      key={h}
                      className="text-left py-3 px-3 text-[10px] uppercase tracking-widest text-stone-500 font-medium"
                    >
                      {h}
                    </th>
                  ),
                ...
                >
                  <td className="py-3 px-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-stone-200 font-medium line-clamp-1">
                          {p.nombre}
                        </p>
                  ...
                  <td className="py-3 px-3">
                    <span
                      className={`text-sm font-body ${p.stock === 0 ? "text-stone-500" : p.stock <= 5 ? "text-amber-400" : "text-stone-300"}`}
                    >
                      {p.stock ?? "—"}
                    </span>
                  </td>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder={
                tipo === "Vino"
                  ? "Ex: Ribera del Duero Reserva"
                  : "Ex: IPA Artesana del Montseny"
              }
          ...
          <div className="border-t border-stone-800/60 pt-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input
                  type="text"
                  value={form.detalles.origen}
                  onChange={(e) => onDetallesChange("origen", e.target.value)}
                  placeholder="Catalunya, Rioja…"
                  className="input-field text-xs"
                />
              </div>
              {tipo === "Vino" && (
                <>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        onDetallesChange("anyada", e.target.value)
                      }
                      placeholder="2021"
                      className="input-field text-xs"
                    />
